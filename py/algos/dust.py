def dust(seq: str, min_len: int = 7, min_score=2.5, threshold_score_difference=0.7, n_chars=False,
    ...
    while _i + 3 < seq_len:
        if print_progress and round((_i + 3) * 100. / seq_len) > done:
            done = int(round((_i + 1) * 100. / (seq_len - 3)))
            print('done : {}%'.format(done))
        _amount[_get_slice_index(seq[_i:_i + 3])] += 1
        if _i > _l:
            _amount[_get_slice_index(seq[_i - _l:_i - _l + 3])] -= 1
            score = _count_dust_score(_amount, _l)
            if score > min_score:
                _add_index = 1
                _add_amount = [0] * (4 ** 3)
                ...
                while True:
                    ...
                    _new_score = _count_dust_score(_amount, _l, additive_array=_add_amount)
                ...
                print_progress and print("add: {}".format(str(_to_cut[-1])))
                print_progress and print("{} : {} || {}".format(seq[_i - _l + 1: _i + 3], score, _new_score))
                _i = _to_cut[-1][1]
                _amount = [0] * (4 ** 3)
        _i += 1
...
def _count_dust_score(occurrence_array: list, l_value: int, additive_array: list = None) -> int:
