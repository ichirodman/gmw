...
def ask_file_name():
    all_slice_matches_files = sorted(os.listdir(RELATIVE_PATH_TO_SLICE_MATCHES_DIR))
        ...
        for line in input_file.read().split('\n'):
            ...
            for query_pos in query_positions:
                for target_pos in target_positions:
                    plot.line(abs(query_pos) + (block_size if target_pos * query_pos < 0 else 0),
                              abs(target_pos),
                              abs(query_pos) + (block_size if target_pos * query_pos >= 0 else 0),
                              abs(target_pos) + block_size,
                              color="#000")
